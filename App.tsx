  ...
  const handleDownload = async (imageUrl: string, fileName: string) => {
    try {
      ...
      try {
        ...
        console.warn(
          "Direct fetch failed, attempting canvas fallback...",
          fetchError
        );
        ...
        await new Promise((resolve, reject) => {
          ...
          img.onload = () => {
            ...
            canvas.toBlob((blob) => {
              if (!blob) {
                reject(
                  new Error("Canvas serialization failed (likely tainted)")
                );
                return;
              }
                    ...
                    >
                      ...
                      >
                        <img
                          ...
                          className={`max-w-full max-h-full object-contain shadow-2xl cursor-grab active:cursor-grabbing transition-all duration-300 ${
                            currentImage.isBlurred ? "blur-lg scale-105" : ""
                          }`}
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              ...
                              <p className="text-white/90">
                                {getModelLabel(currentImage.model)}
                              </p>
                            </div>
                            <div>
                              <span className="block text-white/40 text-[10px] uppercase tracking-wider font-semibold mb-0.5">
                              ...
                              <p className="text-white/90">
                                {imageDimensions
                                  ? `${imageDimensions.width} x ${imageDimensions.height} (${currentImage.aspectRatio})`
